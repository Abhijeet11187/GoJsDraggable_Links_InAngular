...
export class DraggableComponent implements OnInit, OnChanges {
  ...
  save() {
    ...
    // event.target.parentNode.parentNode.childNodes[1].value = myDiagram.model.toJson();
  ...
  load(event) {
    ...
    if (event) {
      ...
      myDiagram.model = go.Model.fromJson(this.dataForm.get('drawingData').value);    } else {
